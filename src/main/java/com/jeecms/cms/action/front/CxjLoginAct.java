...
public class CxjLoginAct {
	 ...
	 */
	@RequestMapping(value = "/cxj/login.jspx", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/cxj/login.jspx", method = RequestMethod.POST)
	public String submit(String username, String password, String processUrl,
		...
		if (!errors.hasErrors()) {
			try {
				...
				if (view != null) {
					...
					model.addAttribute("auth", auth);
					if (Constants.USERTYPE_SUPPORTADMIN.equals(auth.getType())) {
					    model.addAttribute("isRole", "admin");
					}
					return FrontUtils.getTplPath(site.getSolutionPath(), Constants.TPLDIR_MNG_CXJINDEX, "manage");
				}
		...
		if (errors != null && errors.getErrors() != null && errors.getErrors().size() > 0) {
		    model.put("errorMsg", errors.getErrors().get(0));
		}
		return FrontUtils.getTplPath(site.getSolutionPath(), Constants.TPLDIR_MNG_CXJINDEX, "login");

	}

	@RequestMapping(value = "/cxj/logout.jspx")
	public String logout(HttpServletRequest request,
		...
		if (authId != null) {
			...
			session.logout2(request, response, AUTH_KEY);
		}
		return "redirect:/cxj/login.jspx";
	}
